setwd("/home/antortjim/MEGA/Master/Thesis/MaxLFQ")
...
data_df <- data %>% head(6755) %>% do.call(rbind, .)
accessions <- getAccessions(proteinsMaxLFQ)
data_df$Proteins <- rep(accessions, lapply(data, nrow) %>% unlist)
peptides <- data_df

ecoli_fasta <- read.fasta(file = "ecoli.fasta",seqtype = "AA")
human_fasta <- read.fasta(file = "homo_sapiens.fasta",seqtype = "AA")
...
# 
# 
# peptides$prot <- peptides$Leading.razor.protein %>% lapply(., function(x) strsplit(x, split = "-") %>% unlist %>% .[1]) %>% unlist
# 
# peptides$ecoli <- peptides$prot %in% ecoli_prots
# peptides$human <- peptides$prot %in% human_prots
# peptides <- peptides[!(!peptides$ecoli & !peptides$human),]
# table(peptides$ecoli & peptides$human)
# peptides$taxonomy <- ifelse(peptides$human, "Homo sapiens", "Escherichia coli (strain K12)")
# peptides <- peptides %>% select(-human, -ecoli)
...
data_df_subset_wide$peptide <- as.character(data_df_subset_wide$peptide)
data_df_subset_wide <- data_df_subset_wide %>% arrange(peptide)
result_df <- result_df %>% arrange(pep)

all((data_df_subset_wide_solid$peptide == result_df$pep))

write.table(file = "data.tsv", sep = "\t", x = data_df_subset_wide_solid %>% select(-peptide), col.names=T, row.names=F, quote=F)
write.table(file = "peptides_neighborhood.tsv", sep = "\t", x = result_df, col.names=T, row.names=F, quote=F)
