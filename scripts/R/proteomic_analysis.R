...
option_list <- list(
  ...
  make_option(c("--log2fc_threshold"), default=.4)
)
...
RSqM_signif <- read_quant(filename = "../thp1/RSqM_signif.tsv", filetype = "MSqRob")
...
#   write.table(x = RSqM_signif, file = file.path(opt$output_dir, "RSqM_signif_annotated.tsv"), quote = F,row.names = F,col.names = T,sep = "\t")
# }
...
# if(exp_name != "maxlfq") {
 ...
 up_prots <- RSqM_signif %>% filter(signif) %>% filter(estimate > log2fct) %>% .$Protein.IDs %>% strsplit(split = ", ") %>% unlist
 down_prots <- RSqM_signif %>% filter(signif) %>% filter(estimate < -log2fct) %>% .$Protein.IDs %>% strsplit(split = ", ") %>% unlist
 stable_prots <- RSqM_signif %>% filter(signif) %>% filter(abs(estimate) < log2fct) %>% .$Protein.IDs %>% strsplit(split = ", ") %>% unlist
 ...
 write.table(x = res$name, file = file.path(opt$output_dir, "compath.txt"), row.names = F, col.names = F, quote = F)
