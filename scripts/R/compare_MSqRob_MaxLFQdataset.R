...
# 
# roc_data <- RSqM_signif_taxon %>% arrange(qval) %>%
#   filter(signif) %>%
#   select(qval, taxonomy)
# 
# negatives <- roc_data$taxonomy == "Homo sapiens"
...
combined_plot
ggsave(filename = file.path(report_dir, "combined_plot.png"), plot = combined_plot)
ggsave(filename = file.path(report_dir, "ROC.png"), plot = ROC_curve)
ggsave(filename = file.path(report_dir, "PRC.png"), plot = PR_Curve)


RSqM_signif_taxon %>% group_by(Organism) %>% summarise(sum((log2FC > log2(2.5) & qval <  0.05), na.rm=T))


RSqM_table <- RSqM_signif[complete.cases(RSqM_signif),]
RSqM_table <- RSqM_table[(grep(x = RSqM_table$Protein.IDs, pattern = ", ", invert = T)),]
rownames(RSqM_table) <- NULL
RSqM_table <- RSqM_table[sample(x = 1:nrow(RSqM_table), size = 5),]

xtable::xtable(RSqM_table)



# RSqM_signif <- read.table(file.path(export_folder, paste0("RSqM_signif", "_max", ".tsv")),
#                      sep = "\t", header = T)
#            
# source(file.path(home_dir, "thesis/genedata/scripts/R/check_organism.R"))
# 
# organism_check <- check_organism(RSqM_signif$Protein.IDs, split = ", ")
#  
# RSqM_signif_taxon <- left_join(RSqM_signif,
#          data.frame(Protein.IDs = RSqM_signif$Protein.IDs, taxonomy = organism_check[[2]]),
#          by = "Protein.IDs") %>%
#  filter(!is.na(taxonomy))
#            
# ggplot(RSqM_signif_taxon, aes(x = estimate, fill = taxonomy)) + geom_histogram(position = "dodge")
# group_by(RSqM_signif_taxon, taxonomy, signif) %>% summarise(count = n())
